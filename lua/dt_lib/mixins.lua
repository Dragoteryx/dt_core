local MIXIN_CLASS = {}
function DT_Lib.Mixin(class)
  return { [MIXIN_CLASS] = class }
end

local MIXINS = {}
for _, mixin in pairs(DT_Lib.RecursiveInclude("dt_lib/mixins")) do
...
local function MixinApplier(lib)
  return function(tbl, class)
    for _, mixin in ipairs(MIXINS) do
      ...
      if class == mixinClass
      or lib.IsBasedOn(class, mixinClass) then
...
function weapons.OnLoaded()
  SWEPOnLoaded()
  for class, tbl in pairs(weapons.GetList()) do ApplySWEPMixins(tbl.t, class) end
